...
const fetchMoviesByPath = async (path, mediaType = 'movie') => {
...
const fetchTVSeriesByPath = async (path) => {
  const url = buildUrl(path, new URLSearchParams());
  const data = await fetchData(url);
  return processData(data.results).map((item) => ({
    ...item,
    category: 'TV Series',
  }));
};
...
export const fetchPopularShows = async () => {
  return fetchTVSeriesByPath('/tv/popular');
};
